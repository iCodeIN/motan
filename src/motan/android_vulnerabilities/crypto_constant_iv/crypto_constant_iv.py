...
class CryptoConstantIv(categories.ICodeVulnerability):
        ...
        for n, i in enumerate(caller.get_method().get_instructions()):
            ...
            if i.get_output().endswith(
                ...
                if (
                    ...
                    and isinstance(last_invocation_params[1], str)
                ):
                    # The target method invocation was found, with constant parameters.
                    vulnerable_methods[
                        f"{caller.class_name}->{caller.name}{caller.descriptor}"
                    ] = (
                        f'constant IV "{last_invocation_params[1]}" '
                        "parameter passed to IvParameterSpec",
                        " --> ".join(
    ...
    def check_vulnerability(
        ...
        try:
            ...
            paths_to_check = get_paths_to_target_method(target_methods)
            ...
            for path in paths_to_check:
                self.recursive_check_path(
                    path, 0, [], vulnerable_methods, analysis_info
                )
