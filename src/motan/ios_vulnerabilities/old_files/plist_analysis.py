    ...
    if 'NSAppTransportSecurity' in p_list:
        ...
        if 'NSExceptionDomains' in ns_app_trans_dic:
            for key in ns_app_trans_dic['NSExceptionDomains']:
                ...
                if 'NSExceptionRequiresForwardSecrecy' in ns_app_trans_dic['NSExceptionDomains'][key]:
                    if ns_app_trans_dic['NSExceptionDomains'][key]['NSExceptionRequiresForwardSecrecy'] is False:
                        # print(key['NSExceptionRequiresForwardSecrecy'])
                        no_forward_secr.append(key)
                if 'NSExceptionMinimumTLSVersion' in ns_app_trans_dic['NSExceptionDomains'][key]:
                    if ns_app_trans_dic['NSExceptionDomains'][key]['NSExceptionMinimumTLSVersion'] in insecure_tls:
        ...
        if is_source:
            ...
            for dirpath, _dirnames, files in os.walk(src):
                for name in files:
                    ...
                    if '__MACOSX' not in dirpath and os.path.splitext(name)[
                        -1].lower() in extensions and name != app_plist_file:
